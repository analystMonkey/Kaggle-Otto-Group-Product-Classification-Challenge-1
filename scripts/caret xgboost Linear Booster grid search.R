################################################################################
## Caret XGBoost Linear Booster - Grid Search
...
### Grid Search - Linear Booster
XGBoostLinearBoosterGrid <- expand.grid(
        # Number of Boosting Iterations
        nrounds = 10,
        # L1 Regularization term on weights
        alpha = exp(seq(log(1e-1),log(1e2),length.out=10)),  # L1=0; OLS Soultion
        # L2 Regularization term on weights
        lambda = exp(seq(log(1e-1),log(1e2),length.out=10))) # L2=0; OLS Soultion
...
mod1 <- train(target ~ ., data = X_gs, 
              ...
              tuneGrid = XGBoostLinearBoosterGrid,
              trControl = fitControl)
mod1

## 4. Extract Best Parameters
XGBoostLinearBoosterGrid <- data.frame(nrounds=mod1$finalModel$tuneValue[1],
                                       alpha=mod1$finalModel$tuneValue[2],
                                       lambda=mod1$finalModel$tuneValue[3])
