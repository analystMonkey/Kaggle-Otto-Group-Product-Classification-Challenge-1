...
for (i in 1:length(models)) {
        err <- h2o.performance(models[[i]], batch_valid.hex)@metrics$logloss
        if (err < best_err) {
                best_err <- err
                best_model <- models[[i]]
        }
...
train_perf <- h2o.performance(best_model, batch_train60.hex)
train_perf@metrics$cm$table
train_perf@metrics$logloss

## Validation set performance metrics
valid_perf <- h2o.performance(best_model, batch_valid.hex)
...
model <- h2o.gbm(x=predictors, 
                 ...
                 min_rows=10,
                 learn_rate=0.175)
pred   <- as.matrix(predict(model, batch_holdout.hex)[,-1])
